...
# "tillvaxtverket" och tabellen "gastnatter_utlandska_lan". Vill man ha andra namn så kan de skickas med som parameter
...
source("https://raw.githubusercontent.com/Analytikernatverket/hamta_data_playwright/main/hamta_gastnatter_utlandska_lan_tillvaxtverket.R")
...
tabell_db <- if (length(params) < 3) "gastnatter_utlandska_lan" else params[3]       # sparar till schema "ek_bistand_hushall" om det inte finns ett tredje argument medskickat

gastnatter_utlandska_lan <- hamta_gastnatter_utlandska_lan_tillvaxtverket()

postgres_databas_skriv_med_metadata(
  con = uppkoppling_adm(databas),
  inlas_df = gastnatter_utlandska_lan,
  schema = schema_db,
  tabell = tabell_db
)
