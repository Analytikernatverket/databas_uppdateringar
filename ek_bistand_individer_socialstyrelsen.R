...
source("https://raw.githubusercontent.com/Analytikernatverket/hamta_data_playwright/main/hamta_ek_bistand_individer_socialstyrelsen.R")
...
tabell_db <- if (length(argv) < 3) "ek_bistand_individer" else argv[3]   # sparar till schema "ek_bistand_hushall" om det inte finns ett tredje argument medskickat

ek_bistand_individer <- hamta_ek_bistand_individer_socialstyrelsen()

postgres_databas_skriv_med_metadata(
  con = uppkoppling_adm(databas),
  inlas_df = ek_bistand_individer,
  schema = schema_db,
  tabell = tabell_db
)
